...
# Step 1: Load the reduced dataset (PCA dataset)
data_path = '../1.Preprocessing/clean_data_pca.csv'  # Path to PCA reduced dataset
...
output_folder = '../7.Result/'  # Folder to save results
...
for name, clf in classifiers.items():
    ...
    # Plot and save confusion matrix with additional label (PCA)
    plt.figure(figsize=(6, 6))
    sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', cbar=False)
    plt.title(f"Confusion Matrix for {name} (with PCA features)")
    plt.xlabel('Predicted')
    plt.ylabel('Actual')
    
    plt.tight_layout()  # Ensure everything fits well
    cm_image_path = os.path.join(output_folder, f'confusion_matrix_{name}_pca.png')
...
plt.title('ROC Curve for Classifiers (Multi-Class) with PCA features')
plt.legend(loc="lower right")
plt.grid(True)

# Save the ROC curve plot
roc_image_path = os.path.join(output_folder, 'roc_curve_comparison_pca.png')
plt.savefig(roc_image_path)
plt.show()

# Step 7: Create a DataFrame for results and display
results_df = pd.DataFrame(results)
print("\nSummary of Model Performance with PCA features:")
print(results_df)

# Step 8: Save the results to an Excel file
results_excel_path = os.path.join(output_folder, 'model_performance_summary_pca.xlsx')
